package com.br.portfolio.util;

public class CaminhosUrl {


    public static String CAMINHO_FORWARD_CADASTRA_EXPERIENCIA = "/experiencia/exibirCadastrar";

    public static String CAMINHO_INICIO = "index";
    public static String CAMINHO_CADASTRAR_EXPERIENCIA = "experiencia/cadastra";
    public static String CAMINHO_HOME = "home/index";

    public static String CAMINHO_TABLES_HOME = "home/table-basic.html";
    public static String CAMINHO_PROFILE_HOME = "home/pages-profile.html";
    public static String CAMINHO_ICONS_HOME = "home/icon-fontawesome.html";
    public static String CAMINHO_404_HOME = "home/pages-error-404.html";


}
